...
export function walkForward(matches:Match[]){const rows=matches.filter(m=>m.status==='FINISHED'&&m.hg!==null&&m.ag!==null).sort((a,b)=>a.date.localeCompare(b.date));const bins=Array.from({length:10},()=>({n:0,sumP:0,sumY:0}));let n=0,brier=0,logloss=0,naive=0;const split=Math.floor(rows.length*.8),start=Math.floor(rows.length*.6);
 for(let i=start;i<rows.length;i++){const m=rows[i],model=fit(rows,m);if(!model)continue;const ps=[model.probabilities.home,model.probabilities.draw,model.probabilities.away],y=m.hg!>m.ag!?0:m.hg===m.ag?1:2;if(i<split)continue;n++;brier+=ps.reduce((s,p,k)=>s+(p-Number(k===y))**2,0);logloss-=Math.log(Math.max(ps[y],1e-15));naive+=2/3;ps.forEach((p,k)=>{const b=bins[Math.min(9,Math.floor(p*10))];b.n++;b.sumP+=p;b.sumY+=Number(k===y)})}
 return {n,brier:n?brier/n:null,logloss:n?logloss/n:null,naiveBrier:n?naive/n:null,bins:bins.map(b=>({n:b.n,p:b.n?b.sumP/b.n:null,observed:b.n?b.sumY/b.n:null})),roi:null,drawdown:null,bets:0,marketComparison:null,method:'60 % initial, 20 % validation réservée, 20 % test glissant. Paramètres fixes ; résultats rétrospectifs, versions historiques des sources non disponibles.'};}
